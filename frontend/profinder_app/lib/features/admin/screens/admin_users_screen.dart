...
//
// Users Management — Master table (Customers + Professionals combined)
// Features: search, role filter, status filter, sorting, multi-select +
// bulk block/unblock, CSV export (clipboard-based), User Details bottom
// sheet, single ban/unban action.
//
// Backend endpoints:
//   GET   /api/users/                         → all users (customer+professional)
//   GET   /api/users/?role=customer           → customers only
//   GET   /api/users/?role=professional       → professionals only
//   PATCH /api/admin-panel/users/<id>/ban/    → ban / unban
//     body: { "action": "ban" } or { "action": "unban" }
...
enum _SortOption { nameAsc, nameDesc, dateNew, dateOld }
...
class _AdminUsersScreenState extends State<AdminUsersScreen> {
  ...
  // ── Load from backend (all roles, combined) ─────────────────
  ...
  void _applyFilters() {
    ...
    var result = _all.where((u) {
      ...
      final matchRole = switch (_roleFilter) {
        _RoleFilter.all          => true,
        _RoleFilter.customer     => role == 'customer',
        _RoleFilter.professional => role == 'professional',
      };
    ...
    result.sort((a, b) {
      switch (_sortOption) {
        ...
        case _SortOption.dateOld:
          return (a['joined'] ?? '').toString().compareTo((b['joined'] ?? '').toString());
      }
    });
  ...
  Future<void> _bulkSetStatus({required bool block}) async {
    ...
    final confirmed = await _confirmDialog(
      title:   block ? 'Block ${_selectedIds.length} users?' : 'Unblock ${_selectedIds.length} users?',
      message: block
          ? 'Selected users will not be able to login until unblocked.'
          : 'Selected users will be able to login again.',
    ...
    for (final id in _selectedIds.toList()) {
      ...
      if (!block && !isBanned) continue;
      try {
        await _api.patch('/admin-panel/users/${id}/ban/',
            {'action': block ? 'ban' : 'unban'});
    ...
    _showSnack(
      '$success user(s) ${block ? 'blocked' : 'unblocked'}',
      block ? AppColors.error : context.colors.accent,
    );
  }

  // ── Export (CSV → clipboard, no extra package needed) ─────────
  void _exportCsv() {
    ...
    buffer.writeln('Name,Email,Role,Status,City,Joined');
    for (final u in rows) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Export (${rows.length} users)',
  ...
  Widget _buildRoleChips() {
    final filters = [
      (_RoleFilter.all,          'All Roles',      const Color(0xFF374151)),
      (_RoleFilter.customer,     'Customers',       context.colors.primary),
      (_RoleFilter.professional, 'Professionals',   const Color(0xFF7C3AED)),
    ];
    return Container(
      color: Colors.white,
          ...
          return Padding(
            ...
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: isActive ? f.$3.withOpacity(0.15) : Colors.transparent,
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                      color: isActive ? f.$3 : const Color(0xFFE5E7EB)),
                ),
                child: Text(f.$2,
                    style: TextStyle(
                        fontSize: 11,
                        fontWeight: FontWeight.w600,
                        color: isActive ? f.$3 : const Color(0xFF9CA3AF))),
              ),
            ),
          );
        }).toList(),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              _searchCtrl.text.isNotEmpty ? 'No results for "${_searchCtrl.text}"' : 'No users found',
  ...
  Widget _buildUserCard(dynamic user) {
    ...
    final roleColor = role == 'professional' ? const Color(0xFF7C3AED) : context.colors.primary;

    return GestureDetector(
      onTap: _selectionMode ? () => _toggleSelect(id) : () => _showUserDetails(user),
      onLongPress: () {
        if (!_selectionMode) {
      ...
      child: Container(
        ...
        child: Row(
          children: [
                ...
                children: [
                  Row(
                    children: [
                      ...
                      _statusBadge(role.toUpperCase(), roleColor),
                      const SizedBox(width: 4),
                      isBanned
                  ...
                  Row(
                    children: [
                      ...
                      if (joined.isNotEmpty) ...[
                        ...
                        const SizedBox(width: 2),
                        Text('Joined $joined', style: const TextStyle(fontSize: 11, color: Color(0xFF9CA3AF))),
                      ],
                    ],
                  ),
                ],
            ...
            // ── Ban / Unban Button (hidden in selection mode) ──
            if (!_selectionMode)
              GestureDetector(
  ...
  void _showUserDetails(dynamic user) {
    ...
    final hourlyRate = user['hourly_rate']?.toString()    ?? '';
        ...
        builder: (_, scrollCtrl) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _statusBadge(role.toUpperCase(),
                                isPro ? const Color(0xFF7C3AED) : context.colors.primary),
                            ...
                            if (isPro) ...[
                              const SizedBox(width: 6),
                              isVerified
                                  ? _statusBadge('VERIFIED', AppColors.success)
                                  : _statusBadge('UNVERIFIED', AppColors.warning),
                            ],
                          ],
                        ),
                      ],
              ...
              if (isPro) ...[
                _detailRow(Icons.category_outlined, 'Category', category.isEmpty ? '—' : category),
                _detailRow(Icons.payments_outlined, 'Hourly Rate',
                    hourlyRate.isEmpty ? '—' : 'Rs $hourlyRate'),
              ],
              const SizedBox(height: 20),
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(isBanned ? 'Unblock User' : 'Block User',
